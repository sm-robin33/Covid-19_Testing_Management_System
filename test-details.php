...
<!-- Test Information --->
 ...
 <tr>
    <th>Report Status</th> 
    <td><?php if($row['ReportStatus']==NULL):
              echo "Not Processed yet";
          else:
            $id4=intval($row['ReportStatus']);
            $ret4=mysqli_query($con,"SELECT * FROM `status_name` WHERE `id`='$id4'");
            $row3=mysqli_fetch_array($ret4);
            echo $row3['desription'];
...
if($rstatus==5 || $rstatus==1|| $rstatus==2 || $rstatus==3):?>
...
<?php while($result=mysqli_fetch_array($ret)){?>
    ...
    <td><?php 
          $id=intval($result['Status']);
          $ret1=mysqli_query($con,"select * from status_name where id='$id'");
          $row1=mysqli_fetch_array($ret1);
          echo $row1['desription'];?></td>
...
endwhile;
  ?>
            ...
            <?php if($reportstatus==5):?>\  
            <option value=1>On the Way for Collection</option>
            <option value=2>Sample Collected</option>
            <option value=3>Sent to Lab</option>
            <option value=4>Delivered</option>
            <?php elseif($reportstatus==1):?>
            <option value=2>Sample Collected</option>
            <option value=3>Sent to Lab</option>
            <option value=4>Delivered</option>
            <?php elseif($reportstatus==2):?>
             <option value=3>Sent to Lab</option>
            <option value=4>Delivered</option>
             <?php elseif($reportstatus==3):?>
             <option value=4>Delivered</option>
         <?php endif;?>
